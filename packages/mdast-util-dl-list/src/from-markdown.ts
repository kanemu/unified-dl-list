...
export function dlListFromMarkdown(options: DlListFromMarkdownOptions = {}): Extension {
    ...
    // NEW: caller-provided extensions that should also apply inside dt/dd reparse
    const outerExtensions = (options.extensions ?? []) as any[]
    const outerMdastExtensions = (options.mdastExtensions ?? []) as any[]
        ...
        exit: {
            ...
            dlDescContainer(this: CompileContext, token: Token) {
                ...
                const tree = fromMarkdown(raw, {
                    extensions: [...outerExtensions, dlList()],
                    mdastExtensions: [
                        ...outerMdastExtensions,
                        dlListFromMarkdown({
                            ...options,
                            maxDepth: maxDepth - 1
                        })
            ...
            dlTerm(this: CompileContext, token: Token) {
                ...
                if (dt?.type === 'definitionTerm') {
                    ...
                    dt.children = parseInlineToPhrasing(raw, outerExtensions, outerMdastExtensions)
                    delete dt._dlRaw
                }
                this.exit(token)
            },
 ...
 * Inherit extensions/mdastExtensions so ~~ ~~ etc works in dt too.
 */
function parseInlineToPhrasing(
    raw: string,
    extensions: any[],
    mdastExtensions: any[]
): PhrasingContent[] {
